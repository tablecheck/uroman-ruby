...
module Uroman
  class Lattice
    ...
    def initialize(s, uroman, lcode = nil)
      @s = s
      @lcode = lcode
      @lattice = Hash.new { |hash, key| hash[key] = Set.new }
      @max_vertex = s.length
      @uroman = uroman
      @props = {}
      @simple_top_rom_cache = {}
    ...
    def to_s
      ...
      (0...@max_vertex).each do |start|
        @lattice[[start, 'right']].each do |end_vertex|
          @lattice[[start, end_vertex]].each do |edge|
            edges << "[#{start}-#{end_vertex}] #{edge.txt} (#{edge.type})"
          end
        end
      end
      edges.join(' ')
    ...
    def char_is_subjoined_letter?(c)
      @uroman.chr_name(c).include?('SUBJOINED LETTER')
    end

    def char_is_regular_letter?(c)
      char_name = @uroman.chr_name(c)
      char_name.include?('LETTER') && !char_name.include?('SUBJOINED')
    end

    def char_is_letter?(c)
      @uroman.chr_name(c).include?('LETTER')
    end

    def char_is_vowel_sign?(c)
      @uroman.dict_bool[['is-vowel-sign', c]]
    end
    ...
    def is_at_end_of_word?(position)
      ...
      while (start + 1 < @max_vertex) && @uroman.char_is_nonspacing_mark(@s[start]) && @uroman.chr_name(@s[start]).include?('NUKTA')
        start += 1
      end

      (start + 1..@max_vertex).each do |end_pos|
        segment = @s[start...end_pos]
        break unless @uroman.dict_bool[['s-prefix', segment]]

        @uroman.rom_rules[segment].each do |rom_rule|
          rom = rom_rule['t']
    ...
    def is_at_end_of_syllable(position)
      ...
      if @uroman.dict_str[['tone-mark', next_char]]
      ...
      if @uroman.dict_str[['syllable-info', next_char]] == 'written-pre-consonant-spoken-post-consonant'
    ...
    def romanization_by_first_rule(s)
      @uroman.rom_rules[s]&.dig(0, 't')
    end

    def expand_rom_with_special_chars(rom, start, ending, **args)
      orig_start = start
      uroman = self.uroman
      full_string = self.s
      annot = nil
      return [rom, start, ending, nil] if rom.empty?

      prev_char = start >= 1 ? full_string[start - 1] : ''
      first_char = full_string[start]
      last_char = full_string[ending - 1]
      next_char = ending < full_string.length ? full_string[ending] : ''

      if prev_char == "\u2820" && rom.match?(/[a-z]/)
        return [rom[0].upcase + rom[1..], start - 1, ending, 'rom exp']
      end

      if start + 1 == ending && rom == rom.upcase && next_char.match?(/[a-z]/)
        ablation = args[:ablation] || ''
        rom = rom.capitalize unless ablation.include?('nocap')
      end

      if prev_char && ['っ', 'ッ', "\u0A71"].include?(prev_char) &&
        uroman.chr_script_name(prev_char) == uroman.chr_script_name(prev_char) &&
        (m_double_consonant = rom.match(/(ch|[bcdfghjklmnpqrstwz])/))
        rom = if ['っ', 'ッ'].include?(prev_char)
      ...
      if next_char == "\u0300" && uroman.chr_script_name(last_char) == 'Coptic' &&
        !simple_top_romanization_candidate_for_span(orig_start, ending + 1)
        rom = 'e' + rom
        ending += 1
        annot = 'rom exp'
      end

      if ['ゃ', 'ゅ', 'ょ', 'ャ', 'ュ', 'ョ'].include?(next_char) &&
        ...
        !simple_top_romanization_candidate_for_span(orig_start, ending + 1) &&
        !simple_top_romanization_candidate_for_span(start, ending + 1)
        rom = rom[0..-2] + y_rom
        ending += 1
        annot = 'rom exp'
      end
      ...
      last_rom_char = rom[-1]
      if next_char == 'ー' && ['Hiragana', 'Katakana'].include?(uroman.chr_script_name(last_char)) &&
        'aeiou'.include?(last_rom_char)
        return [rom + last_rom_char, start, ending + 1, 'rom exp']
      end

      if uroman.dict_bool[['is-virama', next_char]]
        return [rom, start, ending + 1, 'rom exp']
      end

      rom = rom[1..] if rom.start_with?(' ') && (start == 0 || prev_char == ' ')
      rom = rom[0..-2] if rom.end_with?(' ') && (ending == full_string.length + 1 || next_char == ' ')

      [rom, start, ending, annot]
    end
    ...
    def pick_tibetan_vowel_edge(**args)
      ...
      (0...@max_vertex).each do |start|
        c = s[start]
        if uroman.chr_script_name(c) == 'Tibetan' && char_is_letter_or_vowel_sign(c)
      ...
      tibetan_syllable.each do |tibetan_letter_positions|
        ...
        tibetan_letter_positions.each do |i|
          c = s[i]
          orig_txt += c
          rom = first_non_none(simple_top_romanization_candidate_for_span(i, i + 1), "?")
    ...
    def add_default_abugida_vowel(rom, start, ending, annotation: '')
      ...
      begin
        ...
        if uroman.abugida_cache.key?(key)
          ...
          if (m = rom.match(/([cfghkmnqrstxy]?y)(#{vowels_regex2})-?$/))
            ...
            base_rom, base_rom_plus_vowel = m[1], m[1] + m[2]
            rom = rom[0..-2] if rom.end_with?('-') && (start + 1 == ending) && rom[0].match?(/[A-Za-z]/)
          else
            base_rom = rom
        ...
        next_s_char = ending < s.length ? s[ending] : ''
        next2_s_char = ending + 1 < s.length ? s[ending + 1] : ''
        ...
        return base_rom if uroman.dict_bool[['is-vowel-sign', next_s_char]]
        return base_rom if uroman.dict_bool[['is-medial-consonant-sign', next_s_char]]
        return base_rom if char_is_subjoined_letter(next_s_char)
        return base_rom if uroman.char_is_nonspacing_mark(next_s_char) && uroman.dict_bool[['is-vowel-sign', next2_s_char]]
        return base_rom if uroman.dict_bool[['is-virama', next_s_char]]
        return base_rom if uroman.char_is_nonspacing_mark(next_s_char) && uroman.dict_bool[['is-virama', next2_s_char]]
        return base_rom_plus_vowel if uroman.dict_bool[['is-virama', prev_s_char]]
        return base_rom_plus_vowel if is_at_start_of_word(start) && !rom.match?(/r[aeiou]/)

        if is_at_end_of_word(ending)
        ...
        return base_rom_plus_vowel if uroman.chr_script_name(prev_s_char) != script_name
        return base_rom if uroman.chr_name(last_s_char).include?('VOCALIC')
        return base_rom_plus_vowel if uroman.chr_script_name(next_s_char) == script_name
    ...
    def simple_sorted_romanization_candidates_for_span(start, end_idx)
      ...
      return [] unless uroman.dict_bool[['s-prefix', substring]]

      rom_rule_candidates = []
      uroman.rom_rules[substring].each do |rom_rule|
        rom = rom_rule['t']
    ...
    def simple_top_romanization_candidate_for_span(start, end_idx, simple_search = false)
      ...
      uroman.rom_rules[s[start...end_idx]].each do |rom_rule|
      ...
      if best_rom_rule && (t_at_end_of_syllable = best_rom_rule['t-at-end-of-syllable'])
        is_at_end_of_syllable, rationale = is_at_end_of_syllable(end_idx)
        best_cand = t_at_end_of_syllable if is_at_end_of_syllable
      end
    ...
    def decomp_rom(char_position)
      ...
      if (ud_decomp_s = UD.decomposition(char))
        ...
        if format_comps.any? && !['<super>', '<sub>', '<noBreak>', '<compat>'].include?(format_comps.first) &&
          other_comps.empty? && !decomp_s.empty?
          rom = @uroman.romanize_string(decomp_s, @lcode)
        end

        if rom && UD.numeric(char, nil)
          rom.gsub!('⁄', '/')
          rom = " #{rom}" if char_position >= 1 && UD.numeric(full_string[char_position - 1], nil)
          rom += ' ' if char_position + 1 < full_string.length && UD.numeric(full_string[char_position + 1], nil)
        end
      end
      rom
    end
    ...
    def add_romanization(**args)
      (0...@max_vertex).each do |start|
        ((start + 1)..@max_vertex).each do |end_pos|
          break unless @uroman.dict_bool[['s-prefix', @s[start...end_pos]]]
        ...
        if (0xAC00..0xD7A3).cover?(cp)
          if (rom = @uroman.unicode_hangul_romanization(char))
    ...
    def all_edges(start, end_pos)
      result = []
      (start...end_pos).each do |start2|
        @lattice[[start2, 'right']].sort.reverse_each do |end2|
          if end2 <= end_pos
            result.concat(@lattice[[start2, end2]])
          else
            break
          end
        end
    ...
    def best_edge_in_span(start, ending, skip_num_edge: false)
      edges = @lattice[[start, ending]]
      decomp_edge = nil
      other_edge = nil
      rom_edge = nil
    ...
    def best_right_neighbor_edge(start, skip_num_edge: false)
      @lattice[[start, 'right']].sort.reverse_each do |ending|
        if (best_edge = best_edge_in_span(start, ending, skip_num_edge: skip_num_edge))
          return best_edge
        end
      end
      nil
    ...
    def best_left_neighbor_edge(ending, skip_num_edge: false)
      @lattice[[ending, 'left']].sort.each do |start|
        if (best_edge = best_edge_in_span(start, ending, skip_num_edge: skip_num_edge))
          return best_edge
        end
      end
    ...
    def best_rom_edge_path(start, ending, skip_num_edge: false)
      result = []
      start2 = start
    ...
    def find_rom_edge_path_backwards(start, ending, min_char: nil, return_str: false, skip_num_edge: false)
      result_edges = []
      rom = ''
      end2 = ending
      ...
      while start < end2
        old_end2 = end2
        if (new_edge = best_left_neighbor_edge(end2, skip_num_edge: skip_num_edge))
